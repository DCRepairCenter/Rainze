...
> **设计变更**: 从 6 层简化为 3 层，适配完整帧动画素材。
> 未来如需支持 Live2D 风格分层精灵图，可扩展回 6 层。
...
│   - 完整角色帧动画                                      │
│   - 包含: idle / action / expression / triggered        │
│   - 由行为脚本 (behavior.py) 控制切换                   │
│   - 优先级: 中等                                        │
├─────────────────────────────────────────────────────────┤
...
| 层 | 名称 | 职责 | 素材类型 |
|----|------|------|----------|
| Layer 2 | Overlay | 特效、粒子、指示符 | PNG 序列帧 / 精灵图 |
| Layer 1 | Character | 角色主体动画 | 完整帧 PNG 序列 |
| Layer 0 | Background | 阴影、背景 | 静态 PNG / 可选 |

#### 与旧 6 层的对应关系

| 旧层 | 新层 | 说明 |
|------|------|------|
| Layer 5 (LipSync) | Layer 2 | 简化为特效叠加，不单独处理口型 |
| Layer 4 (Effect) | Layer 2 | 合并到叠加层 |
| Layer 3 (Action) | Layer 1 | 合并到角色层 |
| Layer 2 (Expression) | Layer 1 | 合并到角色层 |
| Layer 1 (Idle) | Layer 1 | 合并到角色层 |
| Layer 0 (Base) | Layer 0 | 改为背景层 |
